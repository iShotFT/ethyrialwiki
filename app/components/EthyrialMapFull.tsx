...
import {
  faGem, faLeaf, faTree, faSkullCrossbones, faMapMarkerAlt, 
  faQuestionCircle, faCity, faLandmark, faCrosshairs, faScroll, faDragon,
...
type Props = {
  mapId: string;
  mapData: { // Data fetched by the parent scene
    id: string;
    title: string;
    // Add extent/bounds info here later if needed from API
  };
  markers: { // Data fetched by the parent scene
    id: string;
    title: string;
...
Logger.debug("misc", `Category Icon Map Keys: ${JSON.stringify(Object.keys(categoryIconMap))}`);

// Helper to create SVG Data URI from FA icon
// Customize color, size etc. here
const createFaDataUri = (iconDef: IconDefinition | undefined, color = 'black') => {
  if (!iconDef) {
  ...
  const svg = 
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${icon[0]} ${icon[1]}" width="24" height="24">
...
const getMarkerStyle = (feature: Feature): Style => {
  ...
  if (!iconStyleCache[cacheKey]) {
    ...
    if (!iconDefinition) {
      Logger.warn(`No icon definition found for categoryId: [${categoryId}], using fallback.`); // Use string only
    }
    
    const iconDataUri = createFaDataUri(iconDefinition || faQuestionCircle, '#D92A2A');
...
const EthyrialMapFull: React.FC<Props> = ({ mapId, mapData, markers }) => {
  // Log received props
  Logger.debug("misc", `[EthyrialMapFull] Received mapId: ${mapId}, mapData: ${mapData ? 'Yes' : 'No'}, markers: ${markers?.length ?? 0}`);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const resolutions = [16, 8, 4, 2, 1]; // Example: 5 zoom levels, 1 is native
      const maxZoom = 8; // Allow zooming IN significantly (arbitrary, adjust for feel)
      ...
      const tileSource = new TileImage({
        ...
        tileUrlFunction: (tileCoord) => {
          ...
          Logger.debug('misc', `Tile Request: OLCoord=[${ol_z_idx},${x},${y_ol}] -> TransformedY=${filenameY} -> FileNameCoords=[${z_filename},${x},${filenameY}]`);
      ...
      const view = new View({
        ...
        zoom: 4, // Start at native resolution (index 4 in the new array)
        resolutions: resolutions,   // Provide all allowed resolutions for view scaling
        extent: mapExtent,
        // constrainResolution: true, // REMOVE this constraint
        minZoom: minZoom, // Set minZoom
        maxZoom: maxZoom, // Keep maxZoom
      });
      ...
      if (popupRef.current && popupContentRef.current && popupCloserRef.current && !overlayRef.current) {
  ...
  useEffect(() => {
    ...
    */
    ...
    Logger.debug("misc", `[EthyrialMapFull] Updating markers: ${markers.length} received.`);
    ...
    const features = markers
      .map(marker => {
        ...
        try {
          ...
          return feature;
           ...
           Logger.error(`[EthyrialMapFull] Error creating feature for marker ${marker.id}`, error);
           return null;
        }
      })
      ...
      Logger.debug("misc", `[EthyrialMapFull] Added ${featuresCreated} features (out of ${markers.length}) to vector source.`);
    // ****************************************************
  ...
  return (
    <MapContainer ref={mapRef}>
      ...
      <PopupContainer ref={popupRef}>
        <PopupCloser ref={popupCloserRef} href="#">&times;</PopupCloser>
        <PopupContent ref={popupContentRef}></PopupContent>
      </PopupContainer>
    </MapContainer>
  );
};
