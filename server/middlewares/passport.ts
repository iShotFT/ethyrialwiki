      ...
      .authenticate(
        ...
        async (
          ...
          if (err) {
            ...
            if (err.id) {
              ...
              const state = stateInfo?.host
                ...
                ? parseState(ctx.cookies.get("state")!)
                : undefined;
          ...
          if (env.DEBUG_AUTH) {
            Logger.debug(
              "authentication",
              `[passportMiddleware Callback] Response headers BEFORE signIn redirect: ${JSON.stringify(
      ...
      .then(() => {
        ...
        if (ctx.status === 302) {
          ...
          if (env.DEBUG_AUTH) {
            Logger.debug(
              "authentication",
              `[passportMiddleware Initial Redirect] Response headers SENT for redirect TO provider: ${JSON.stringify(
                ctx.response.headers
              )}`
            );
          }
