...
export class StateStore {
  ...
  store = (ctx: Context, callback: StateStoreStoreCallback) => {
    ...
    try {
      ...
      Logger.error(
        "Failed to parse PUBLIC_URL, falling back to ctx.hostname",
        e,
    ...
    if (env.DEBUG_AUTH) {
      Logger.debug(
        "authentication",
        `[StateStore.store] Checking getCookieDomain condition...`
      ); // Simplified this log
      Logger.debug(
        "authentication",
        `[StateStore.store] Using Host for state (from PUBLIC_URL): [${hostForState}]`
      );
      Logger.debug(
        "authentication",
        `[StateStore.store] Calculated Cookie Domain (Specific for state): [${stateCookieDomain}]`
      );
      Logger.debug(
        "authentication",
        `[StateStore.store] Setting 'state' cookie with state: [${state}]`
      );
    }
    ...
    if (env.DEBUG_AUTH) {
      Logger.debug(
        "authentication",
        `[StateStore.store] Manually Appended Set-Cookie header: [${cookieString}]`
      );
    }
    ...
    */
    ...
    Logger.debug(
      "authentication",
      `[StateStore.store] Checking getCookieDomain condition: isDev=${isDev}, endsWith=${endsWith}, hostForState=[${hostForState}]`
    );
    // ***************************
  ...
  verify = (
    ...
    if (env.isDevelopment && env.DEBUG_AUTH) {
      // Log only if DEBUG_AUTH is also true
      Logger.warn(
        "[StateStore.verify] DEVELOPMENT MODE: Bypassing state verification."
      );
    }
    if (env.isDevelopment) {
      // Keep bypass logic but don't log unless DEBUG_AUTH
      const bypassState = buildState(
        env.PUBLIC_URL.split("//")[1]?.split(":")[0] || "localhost",
        providedToken,
        Client.Web
      ); // Construct a plausible state
      const { host: hostFromState, client: clientFromState } =
        parseState(bypassState);
      // @ts-expect-error Type in library is wrong, and we are adding extra args
      return callback(null, true, bypassState, {
        host: hostFromState,
        client: clientFromState,
      });
    }
    ...
    const originalHost =
      ctx.get("X-Forwarded-Host") || ctx.get("Host") || ctx.hostname;
    const stateCookieDomain = originalHost.split(":")[0]; // Get hostname without port

    if (env.DEBUG_AUTH) {
      Logger.debug(
        "authentication",
        `[StateStore.verify] Verifying on host [${originalHost}] (using specific domain for state cookie: [${stateCookieDomain}])`
      );
      Logger.debug(
        "authentication",
        `[StateStore.verify]   State from Cookie: [${stateFromCookie}]`
      );
      Logger.debug(
        "authentication",
        `[StateStore.verify]   Token from URL:    [${providedToken}]`
      );
    }

    if (!stateFromCookie) {
      if (env.DEBUG_AUTH) {
        Logger.warn("[StateStore.verify] State cookie not found.");
      }
      return callback(
    ...
    const {
      ...
      host: hostFromState,
      client: clientFromState,
    } = parseState(stateFromCookie);
    if (env.DEBUG_AUTH) {
      Logger.debug(
        "authentication",
        `[StateStore.verify]   Token from Cookie: [${tokenFromCookie}]`
      );
    }
    ...
    if (!tokenFromCookie || tokenFromCookie !== providedToken) {
      if (env.DEBUG_AUTH) {
        Logger.warn("[StateStore.verify] Token mismatch or missing.");
      }
    ...
    if (env.DEBUG_AUTH) {
      Logger.debug(
        "authentication",
        "[StateStore.verify] State verification successful."
      );
    }
    // Pass back the original host and client from the state
    // @ts-expect-error Type in library is wrong, and we are adding extra args
    callback(null, true, stateFromCookie, {
      host: hostFromState,
      client: clientFromState,
    });
  };
