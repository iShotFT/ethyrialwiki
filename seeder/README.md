...
- `input/`: **(Gitignored - Add manually)** Place necessary input files here:
  ...
  - `cleaned_doodad.json`: JSON file containing doodad resource data (primarily trees).
...
4.  **Database Seeding:** Runs the core data, map data (using the generated JSON), item/icon, resource seeding, doodad resource seeding (trees), and custom domain seeding functions within a database transaction.
...
- `--skip-doodad-resources`: Skips doodad resource (primarily trees) seeding. Useful when you don't need to update tree resources.
...
# Run only standard resources (skip doodad/tree resources)
yarn seed:all --resources-mode standard

# Run only doodad/tree resources (skip standard resources)
yarn seed:all --resources-mode doodad
...
### Doodad Resource Seeding
A new seeder for doodad resources has been added that processes tree resources and other doodads from a cleaned_doodad.json file:
- Handles tree resources that are skipped by the standard resource seeder
- Processes resources from a structured JSON format with map coordinates and tags
- Normalizes resource names and identifies resource types based on tags
- Supports batch processing with configurable batch sizes
- Can be run independently with the `seed:game-resources` command
